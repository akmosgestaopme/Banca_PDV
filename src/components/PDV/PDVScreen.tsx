...
import { Search, ShoppingCart, X, Plus, Minus, Calculator, Trash2, Package, Barcode, DollarSign, Clock, User, Zap, TrendingUp, ArrowLeft, AlertCircle, Building, Monitor, RefreshCw, CheckCircle, XCircle, Tag, FileText, Eye, EyeOff } from 'lucide-react';
...
const PDVScreen: React.FC<PDVScreenProps> = ({ onBack }) => {
  ...
  const shortcuts = [
    ...
    { key: 'F7', label: 'Pesquisar', action: () => searchInputRef.current?.focus(), color: '#0d214f', icon: Search },
    { key: 'F8', label: 'Adicionar', action: () => selectedProduct && addToCart(selectedProduct, quantity), color: '#7c3aed', icon: Package },
    { key: 'F10', label: 'Finalizar', action: () => cart.length > 0 && setShowPaymentModal(true), color: '#ea580c', icon: DollarSign },
